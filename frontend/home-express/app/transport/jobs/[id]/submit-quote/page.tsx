...
async function getSuggestedPrice(id: string, cookieHeader: string): Promise<SuggestedPriceResponse | null> {
  const response = await fetch(buildApiUrl(`/transport/bookings/${id}/suggested-price`), {
    headers: {
      Accept: "application/json",
      ...(cookieHeader ? { Cookie: cookieHeader } : {}),
    },
  ...
  if (!response.ok) {
    console.error("Failed to load suggested price", response.status)
    return null
  }
...
export default async function SubmitQuotePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [booking, items, suggestedPriceData] = await Promise.all([
    getBooking(id, cookieHeader),
    getBookingItems(id, cookieHeader),
    getSuggestedPrice(id, cookieHeader),
  ])
  ...
  return (
    <DashboardLayout navItems={navItems} title="Gởi báo giá">
      <div className="container max-w-7xl py-10 space-y-6">
        ...
        <QuotationForm booking={bookingWithItems} suggestedPrice={suggestedPriceData?.data || null} />
      </div>
    </DashboardLayout>
  )
}

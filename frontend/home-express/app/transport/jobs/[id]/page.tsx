...
import type { SuggestedPriceResponse } from "@/types"
...
export default function JobDetailPage() {
  ...
  const [suggestedPrice, setSuggestedPrice] = useState<SuggestedPriceResponse | null>(null)
  ...
  const [showQuotationForm, setShowQuotationForm] = useState(false)
	...
	useEffect(() => {
		if (!bookingId || isLoading || isError || !booking) return
		...
		const loadSuggested = async () => {
			try {
				...
				const result = await apiClient.getSuggestedPrice(bookingId)
				if (isMounted) {
					setSuggestedPrice(result)
				}
			} catch (error) {
				console.error("[Stage3.4] Failed to fetch suggested price", error)
				if (isMounted) {
	...
	}, [bookingId, isLoading, isError, booking])

...
const STATUS_FLOW = [
  { key: "CONFIRMED", label: "Đã xác nhận", icon: CheckCircle2, action: "start" },
  { key: "IN_PROGRESS", label: "Đang thực hiện", icon: Truck, action: "complete" },
  { key: "COMPLETED", label: "Hoàn thành", icon: CheckCircle2, action: null },
]
...
export default function ActiveJobDetailPage({ params }: ActiveJobDetailPageProps) {
  ...
  // SSE for real-time updates (disabled until backend endpoint is ready)
  // useSSE(
  ...
  const handleCompleteJob = async () => {
    setIsUpdating(true)
    try {
      await apiClient.completeJob(bookingId, {})
  ...
  const currentAction = STATUS_FLOW[currentStatusIndex]?.action
              ...
              <div className="mt-6 pt-6 border-t">
                {currentAction === "start" && (
                  <Button
                    ...
                    size="lg"
                  >
                    {isUpdating ? "Đang xử lý..." : "Bắt đầu công việc"}
                  </Button>
                )}
                {currentAction === "complete" && (
                  <Button
                    ...
                    size="lg"
                  >
                    {isUpdating ? "Đang xử lý..." : "Hoàn thành công việc"}
                  </Button>
                )}
              </div>
            )}

...
import type {
  ...
  InitiateRemainingPaymentRequest,
  InitiateRemainingPaymentResponse,
  BankInfo,
  TransportExportJob,
  TransportStats,
...
type BookingStatus = "PENDING" | "QUOTED" | "CONFIRMED" | "CONFIRMED_BY_CUSTOMER" | "IN_PROGRESS" | "COMPLETED" | "REVIEWED" | "CANCELLED"
...
class ApiClient {
   ...
   * and maps them to the TransportStats type used by the dashboard UI.
   */
  async getTransportStats(): Promise<TransportStats> {
    const dto = await this.request<{
      ...
      in_progress_bookings: number | string | null
    ...
    return {
      total_income: this.toNumber(dto.total_income, 0),
      completed_bookings: this.toNumber(dto.completed_bookings, 0),
      in_progress_bookings: this.toNumber(dto.in_progress_bookings, 0),
      average_rating: this.toNumber(dto.average_rating, 0),
      completion_rate: this.toNumber(dto.completion_rate, 0),
      pending_quotations: this.toNumber(dto.pending_quotations, 0),
      monthly_revenue:
        dto.monthly_revenue?.map((item) => ({
          month: item.month,
          revenue: this.toNumber(item.revenue, 0),
        })) ?? [],
    }
  }
  ...
  /**
   * Initiates wallet transaction export for the current transport
   */
  async requestTransportWalletExport() {
    return this.request<{ data: TransportExportJob }>("/transport/reports/wallet", {
      method: "POST",
    }).then((res) => res.data)
  }

  /**
   * Polls the status of a transport export job
   */
  async getTransportExportJob(jobId: number) {
    return this.request<{ data: TransportExportJob }>(`/transport/reports/${jobId}`).then((res) => res.data)
  }

  /**
   * Downloads an export job file
   */
  async downloadTransportExportJob(jobId: number, token: string) {
    const url = buildApiUrl(`/transport/reports/${jobId}/download?token=${encodeURIComponent(token)}`)
    ...
    if (!response.ok) {
      throw new Error("KhA'ng th��� tA?i file bA�o cA�o")
    }
  ...
  /**
   * Fetches recent quotations for the transport dashboard
   */
  async getTransportQuotations(): Promise<TransportQuotationSummary[]> {
    return this.request<TransportQuotationSummary[]>("/transport/quotations")
  }
  ...
  // ---------------------------------------------------------------------------
  // Transport rate cards (Stage 3.2)
  // ---------------------------------------------------------------------------

  async getTransportRateCards(): Promise<RateCard[]> {
    return this.request<RateCard[]>("/transport/rate-cards")
  }

  async createOrUpdateRateCard(data: RateCardRequest): Promise<RateCard> {
    return this.request<RateCard>("/transport/rate-cards", {
      method: "POST",
      body: JSON.stringify(data),
    })
  ...
  async deleteRateCard(rateCardId: number): Promise<void> {
    return this.request<void>(`/transport/rate-cards/${rateCardId}`, {
      method: "DELETE",
    })
  }

  async getReadyToQuoteStatus(): Promise<ReadyToQuoteStatus> {
    return this.request<ReadyToQuoteStatus>("/transport/ready-status")
  }

  async getSuggestedPrice(bookingId: number): Promise<SuggestedPriceResponse> {
    return this.request<SuggestedPriceResponse>(`/transport/bookings/${bookingId}/suggested-price`)
  }
  ...
  async getVehicleTypePricing() {
    ...
    }>("/transport/pricing/vehicle-types")
  }

  /**
   * Set pricing by vehicle type (Option A)
   */
  async setVehicleTypePricing(data: {
    vehicleType: string
    basePrice: number
    perKmFirst4km: number
    perKm5To40km: number
    ...
    return this.request<{ success: boolean; message: string }>("/transport/pricing/vehicle-types", {
      method: "POST",
      body: JSON.stringify(data),
    })
  }
  ...
  /**
   * Get active categories for transport partners
   */
  async getTransportCategories(params?: { isActive?: boolean }) {
    const queryParams = new URLSearchParams()
    ...
    const endpoint = queryString ? `/transport/categories?${queryString}` : "/transport/categories"
  ...
  /**
   * Upload evidence photos for a booking
   */
   ...
   */
  async initiateDeposit(data: InitiateDepositRequest) {
    ...
    return this.request<InitiateDepositResponse>("/customer/payments/deposit/initiate", {
      method: "POST",
      body: JSON.stringify(payload),
    })
  }
   ...
   */
  async initiateRemainingPayment(data: InitiateRemainingPaymentRequest) {
    const { paymentMethod, ...rest } = data
    ...
    return this.request<InitiateRemainingPaymentResponse>(
   ...
   */
  async getBankInfo() {
    return this.request<BankInfo>("/customer/payments/bank-info")
  }
  ...
  async getDisputeById(disputeId: number) {
    return this.request<Dispute>(`/customer/disputes/${disputeId}`)
  }
  ...
  async addDisputeMessage(disputeId: number, messageData: {
    ...
    return this.request<DisputeMessage>(`/customer/disputes/${disputeId}/messages`, {
      method: 'POST',
  ...
  /**
   ...
   // ============================================================================
   // TRANSPORT SETTLEMENT METHODS
   // ============================================================================

   /**
    * Get transport's settlements with optional status filter
    */
   async getSettlements(params?: {
     ...
     return this.get<{
       ...
       totalItems: number
       totalPages: number
     }>('/transport/settlements', params)
   }

   /**
    * Get settlement details by ID
    */
   async getSettlementDetails(settlementId: number) {
     return this.get<{
       settlementId: number
       bookingId: number
       transportId: number
       ...
       updatedAt: string
       notes?: string
     }>(`/transport/settlements/${settlementId}`)
   }
   ...
   // ============================================================================
   // TRANSPORT PAYOUT METHODS
   // ============================================================================

   /**
    * Get transport's payouts
    */
   ...
   async getPayoutDetails(payoutId: number) {
     ...
     }>(`/transport/payouts/${payoutId}`)
   }

   /**
    * Get pending settlements available for payout
    */
   async getPendingSettlementsForPayout() {
     return this.get<{
       settlements: Array<{
         settlementId: number
         bookingId: number
       ...
       count: number
       totalAmount: number
     }>('/transport/payouts/pending')
   }

   // ============================================================================
   // TRANSPORT DISPUTE METHODS
   // ============================================================================

   /**
    * Get transport's disputes
    */
   async getTransportDisputes(params?: {
     status?: string
     page?: number
     size?: number
   }) {
     return this.get<{
       ...
       totalItems: number
       totalPages: number
     }>('/transport/disputes', params)
   }

   /**
    * Get dispute details
    */
   async getTransportDisputeDetails(disputeId: number) {
     return this.get<{
       disputeId: number
       bookingId: number
       settlementId?: number
       ...
       createdAt: string
       updatedAt: string
     }>(`/transport/disputes/${disputeId}`)
   }

   /**
    * Get wallet transaction history for transport
    */
   async getWalletTransactions(params?: {
     ...
     }>('/transport/wallet/transactions', params)
   }

   // ============================================================================
   // CUSTOMER SAVED ITEMS METHODS
   // ============================================================================
   ...
   async getSavedItems() {
     ...
     }>('/api/v1/customer/saved-items')
   }

   /**
    * Save items to storage
    */
   async saveItemsToStorage(items: Array<{
     name: string
     brand: string | null
     model: string | null
     categoryId: number | null
     ...
     return this.post<{
       count: number
       message: string
     }>('/api/v1/customer/saved-items', { items })
   }
    ...
    */
   async deleteSavedItem(id: number) {
     return this.delete<{ message: string }>(`/api/v1/customer/saved-items/${id}`)
   }
    ...
    */
   async deleteSavedItems(ids: number[]) {
     return this.delete<{ message: string }>('/api/v1/customer/saved-items', { ids })
   }
    ...
    */
   async deleteAllSavedItems() {
     return this.delete<{ message: string }>('/api/v1/customer/saved-items/all')
   }
   }

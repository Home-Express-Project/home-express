...
import type { BookingDetailResponse, SuggestedPriceResponse } from "@/types"
...
export function QuotationForm({ booking, bookingId, suggestedPrice, loadingSuggested, onCancel, onSuccess }: QuotationFormProps) {
  ...
  // Stage 3.4: apply suggested price from dynamic pricing service when available
  useEffect(() => {
    if (!suggestedPrice) return

    const breakdown = suggestedPrice.price_breakdown

    if (breakdown) {
      setBasePrice(Math.round(breakdown.base_price ?? 0))
      setDistancePrice(Math.round(breakdown.distance_price ?? 0))
      setItemHandlingPrice(Math.round(breakdown.time_price ?? 0))
    }

    if (typeof suggestedPrice.suggested_price === "number") {
      setReferencePrice(Math.round(suggestedPrice.suggested_price))
    }
  }, [suggestedPrice])
  ...
  const suggestedTotal = suggestedPrice?.suggested_price ?? 0
  const suggestedDeltaPercent = suggestedTotal > 0 ? ((totalPrice - suggestedTotal) / suggestedTotal) * 100 : 0
  const suggestedDeltaAbs = Math.abs(suggestedDeltaPercent)
        ...
        <CardContent className="space-y-3">
          ...
          {suggestedTotal > 0 && (
            <div className="mt-4 pt-4 border-t space-y-2">
              <div className="flex justify-between text-sm">
                <span className="font-medium">Suggested price (AI)</span>
                <span className="font-medium">{formatVND(suggestedTotal)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span>Your quote vs suggested</span>
                <span className={suggestedDeltaAbs > 30 ? "text-destructive font-semibold" : suggestedDeltaAbs > 10 ? "text-yellow-600 font-semibold" : "text-green-600 font-semibold"}>
                  {suggestedDeltaPercent > 0 ? "+" : ""}
                  {suggestedDeltaPercent.toFixed(1)}%
                </span>
              </div>
              <Alert
                variant={suggestedDeltaAbs > 30 ? "destructive" : "default"}
                className={suggestedDeltaAbs > 30 ? "border-red-500 bg-red-50" : suggestedDeltaAbs > 10 ? "border-yellow-500 bg-yellow-50" : "border-green-500 bg-green-50"}
              >
                <AlertCircle className="h-4 w-4" />
                <AlertDescription className={suggestedDeltaAbs > 30 ? "text-red-700" : suggestedDeltaAbs > 10 ? "text-yellow-700" : "text-green-700"}>
                  {suggestedDeltaAbs <= 10 && "Your quote is close to the suggested price."}
                  {suggestedDeltaAbs > 10 && suggestedDeltaAbs <= 30 && "Moderate deviation from suggested price."}
                  {suggestedDeltaAbs > 30 && "Large deviation may reduce acceptance rate."}
                </AlertDescription>
              </Alert>
            </div>
          )}
